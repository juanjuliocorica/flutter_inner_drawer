...
class InnerDrawerState extends State<InnerDrawer> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _controller = AnimationController(value: 1, duration: widget.duration ?? _kBaseSettleDuration, vsync: this)
  ...
  void _animationChanged() {
    ...
    if (widget.colorTransitionChild != null) _colorTransitionChild = ColorTween(begin: widget.colorTransitionChild!.withOpacity(0.0), end: widget.colorTransitionChild);

    if (widget.colorTransitionScaffold != null) _colorTransitionScaffold = ColorTween(begin: widget.colorTransitionScaffold, end: widget.colorTransitionScaffold!.withOpacity(0.0));
  ...
  void _move(DragUpdateDetails details) {
    ...
    else if (delta < 0 && _controller.value == 1 && _rightChild != null) _position = InnerDrawerDirection.end;
    ...
    final bool opened = _controller.value < 0.5 ? true : false;
    if (opened != _previouslyOpened && widget.innerDrawerCallback != null) widget.innerDrawerCallback!(opened);
    _previouslyOpened = opened;
  }
  ...
  Widget _scaffold() {
    ...
    Widget container = Container(
        ...
        child: widget.borderRadius != 0 ? ClipRRect(borderRadius: BorderRadius.circular((1 - _controller.value) * widget.borderRadius), child: scaffoldChild) : scaffoldChild);
    ...
    if (widget.offset != null && (widget.offset.top > 0 || widget.offset.bottom > 0)) {
      final double translateY = MediaQuery.of(context).size.height * (widget.offset.top > 0 ? -widget.offset.top : widget.offset.bottom);
  ...
  Widget? _trigger(AlignmentDirectional alignment, Widget? child) {
    ...
    if (Directionality.of(context) == TextDirection.rtl) dragAreaWidth = drawerIsStart ? padding.right : padding.left;
            ...
            child: RepaintBoundary(
              child: Stack(
                children: <Widget?>[
                  ...
                  Align(
                    alignment: _drawerOuterAlignment!,
                    child: Align(alignment: _drawerInnerAlignment!, widthFactor: wFactor, child: RepaintBoundary(child: _scaffold())),
                  ),
...
class IDOffset {
  ...
  const IDOffset.only({
    ...
    this.right = 0.0,
    this.bottom = 0.0,
  })  : assert(top >= 0.0 && top <= 1.0 && left >= 0.0 && left <= 1.0 && right >= 0.0 && right <= 1.0 && bottom >= 0.0 && bottom <= 1.0),
